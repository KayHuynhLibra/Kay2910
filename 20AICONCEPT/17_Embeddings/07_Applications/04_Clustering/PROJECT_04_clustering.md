...
**Tiếng Việt:** Xây dựng một hệ thống hoàn chỉnh sử dụng 04 Clustering để giải quyết bài toán thực tế.

**English:** Build a complete system using 04 Clustering to solve a real-world problem.

## 🎯 Mục Tiêu / Objectives

1. **Hiểu sâu về 04 Clustering** / Deep understanding of 04 Clustering
...
def create_sample_dataset():
    '''
    Create sample dataset for 04 Clustering project
    '''
    np.random.seed(42)
    n_samples = 1000
...
def prepare_project_data(df):
    '''
    Prepare data for 04 Clustering project
    '''
...
```python
def develop_04_clustering_model(X, y):
    '''
    Develop 04 Clustering model for project
    '''
...
```python
def optimize_04_clustering_model(X, y):
    '''
    Optimize 04 Clustering model using hyperparameter tuning
    '''
