...
**Tiếng Việt:** Xây dựng một hệ thống hoàn chỉnh sử dụng 04 Softmax để giải quyết bài toán thực tế.

**English:** Build a complete system using 04 Softmax to solve a real-world problem.

## 🎯 Mục Tiêu / Objectives

1. **Hiểu sâu về 04 Softmax** / Deep understanding of 04 Softmax
...
def create_sample_dataset():
    '''
    Create sample dataset for 04 Softmax project
    '''
    np.random.seed(42)
    n_samples = 1000
...
def prepare_project_data(df):
    '''
    Prepare data for 04 Softmax project
    '''
...
```python
def develop_04_softmax_model(X, y):
    '''
    Develop 04 Softmax model for project
    '''
...
```python
def optimize_04_softmax_model(X, y):
    '''
    Optimize 04 Softmax model using hyperparameter tuning
    '''
