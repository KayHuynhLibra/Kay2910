...
**Tiếng Việt:** Xây dựng một hệ thống hoàn chỉnh sử dụng 04 Catboost để giải quyết bài toán thực tế.

**English:** Build a complete system using 04 Catboost to solve a real-world problem.

## 🎯 Mục Tiêu / Objectives

1. **Hiểu sâu về 04 Catboost** / Deep understanding of 04 Catboost
...
def create_sample_dataset():
    '''
    Create sample dataset for 04 Catboost project
    '''
    np.random.seed(42)
    n_samples = 1000
...
def prepare_project_data(df):
    '''
    Prepare data for 04 Catboost project
    '''
...
```python
def develop_04_catboost_model(X, y):
    '''
    Develop 04 Catboost model for project
    '''
...
```python
def optimize_04_catboost_model(X, y):
    '''
    Optimize 04 Catboost model using hyperparameter tuning
    '''
