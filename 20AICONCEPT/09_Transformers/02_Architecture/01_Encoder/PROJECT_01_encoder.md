...
**Tiếng Việt:** Xây dựng một hệ thống hoàn chỉnh sử dụng 01 Encoder để giải quyết bài toán thực tế.

**English:** Build a complete system using 01 Encoder to solve a real-world problem.

## 🎯 Mục Tiêu / Objectives

1. **Hiểu sâu về 01 Encoder** / Deep understanding of 01 Encoder
...
def create_sample_dataset():
    '''
    Create sample dataset for 01 Encoder project
    '''
    np.random.seed(42)
    n_samples = 1000
...
def prepare_project_data(df):
    '''
    Prepare data for 01 Encoder project
    '''
...
```python
def develop_01_encoder_model(X, y):
    '''
    Develop 01 Encoder model for project
    '''
...
```python
def optimize_01_encoder_model(X, y):
    '''
    Optimize 01 Encoder model using hyperparameter tuning
    '''
